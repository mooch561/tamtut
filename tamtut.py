...
class TamLetter:
    ...
    def __init__(self, t, p):
        self.type = t
        self.pos = p
        filename = "images" + "\\" + self.type + str(self.pos) + ".JPG"
...
def get_letters(type):
    prefix = "images/" + type + "*" + ".JPG"
    files = [x for x in glob.glob(prefix)]
    
    letter_objs = []
    for f in files:
        p = f.replace("images\\", "").replace(".JPG", "").replace(type, "")
...
### main ###
os.chdir('D:\\MyCode\\python\\tamtut')
currdir = os.getcwd()
...
canvas.pack()
img = ImageTk.PhotoImage(Image.open('images/header.JPG'))
